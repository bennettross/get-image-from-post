...
function thisismyurl_get_image_from_post($options='') {
	...
	foreach ($options as $option) {
		...
		if ($parts[1]) {$ns_options[$parts[0]] = $parts[1];}
	}
	...
	if ($content) {
		...
		if ($strip) {$thisimage[$ns_options['image']] = thisismyurl_strip_tags_attributes($thisimage[$ns_options['image']],"img","src");} 
		if ($show) {echo $thisimage[$ns_options['image']];} else {return  $thisimage[$ns_options['image']];}
	}
}
